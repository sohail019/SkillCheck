...
function Verify() {
  ...
  useEffect(function () {
    console.log({ accountId: ctx.state?.accountId });
    if (ctx.state?.accountId) history.push("/");
  }, []);
              ...
              <button
                onClick={async () => {
                  if (state.connected) {
                    ...
                    const signupRes = await signUp(ctx, email, name, accType);
                    console.log({ signupRes });
                    if (signupRes) history.push("/");
